...
export default class CanvasFullScreen extends Component {
    ...
    animate = (length: number, amplitude: number, increment: number): void => {
        let bg: any = this.hexToRgb(this.context.theme.toneOne);
        requestAnimationFrame(() => this.animate(length, amplitude, increment));
    ...
    componentDidMount = (): void => {
        ...
        this.animate(length, amplitude, increment);
    };

    componentWillUnmount = (): void => {
        window.removeEventListener('resize', this.handleResize);   
        this.animate = (length: number, amplitude: number, increment: number): void => {
            return;
        }
    };
